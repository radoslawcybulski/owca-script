...
namespace OwcaScript::Internal {
	...
	std::unique_ptr<AstClass> AstCompiler::compile_class_raw()
	{
		...
		auto c = std::make_unique<AstClass>(line, std::string{ class_name }, std::string{ full_name }, std::move(base_classes), std::move(members), std::move(variable_names), all_variable_names, use_native);
		return c;
	}
	...
	std::unique_ptr<AstStat> AstCompiler::compile_while(std::string_view loop_ident)
	{
		...
		return std::make_unique<AstWhile>(line, control_depth, std::move(val), std::move(body));
	}
