...
namespace owca { namespace __owca__ {
	...
	opcode_validator::boolean_result op_log_and_validate(opcode_validator &oe)
	{
		...
		if (!oe.push_local_stack_data_size(sizeof(op_flow_log_and))) return false;
		...
		if (!oe.pop_local_stack_data_size(sizeof(op_flow_log_and))) return false;
		if (!oe.push_temporary_variable()) return false;
		oe.execution_can_continue=oe.continue_opcodes=true;
		return true;
	}

	bool op_flow_log_and::update_bool_value(vm_execution_stack_elem_internal &oe)
	{
	...
	bool op_flow_log_and::resume_fin(vm_execution_stack_elem_internal &oe)
	{
		switch(mode) {
		case 0:
	...
	RCLMFUNCTION bool op_log_and_flow(vm_execution_stack_elem_internal &oe)
	{
		op_flow_log_and *ff;
