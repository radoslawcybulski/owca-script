...
namespace owca { namespace __owca__ {
	...
	RCLMFUNCTION bool op_lshift_flow(vm_execution_stack_elem_internal &oe)
	{
		...
		if (params[0].get_int(i1) && params[1].get_int(i2)) {
			if (i2<0) {
				oe.vm->raise_invalid_integer(OWCA_ERROR_FORMAT("cant left shift by negative amount"));
			...
			oe.prepare_call_operator_stack(E_LSHIFT);
			oe.r=returnvalueflow::CALL_FUNCTION_CONTINUE_OPCODES;
		}
		return false;
	}
