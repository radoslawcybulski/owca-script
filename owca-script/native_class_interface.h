...
namespace OwcaScript {
	...
	struct NativeClassInterface {
		...
		virtual void gc_mark_members(const void* ptr, size_t s, const OwcaVM &vm, GenerationGC generation_gc) = 0;
		virtual size_t native_storage_size() = 0;
		virtual bool get_member(const OwcaVM &vm, std::string_view, std::span<char> native_storage, OwcaValue &) { return false; }
		virtual bool set_member(const OwcaVM &vm, std::string_view, std::span<char> native_storage, const OwcaValue &) { return false; }
	};
	template <typename T> struct NativeClassInterfaceSimpleImplementation : public NativeClassInterface {
		...
		void gc_mark_members(const void* ptr, size_t s, const OwcaVM &vm, GenerationGC generation_gc) override { gc_mark_value(vm, generation_gc, *(const T*)ptr); }
